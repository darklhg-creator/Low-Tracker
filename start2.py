...
import json
import numpy as np
import time  # 👈 시간 측정을 위해 추가되었습니다.
from concurrent.futures import ThreadPoolExecutor

warnings.filterwarnings('ignore')

# ✅ 디스코드 웹후크 URL (사용자님 주소 적용)
...
def main():
    ...
    print(f"🚀 병렬 분석 엔진 가동... 목표 시간 5분 이내!")
    ...
    # 병렬 작업 설계 (max_workers=10으로 안정성 확보)
    ...
    # 디스코드 메시지 구성
    if not final_picks:
        msg = f"📅 {end_date.strftime('%Y-%m-%d')} | 모든 조건을 만족하는 정예 종목이 없습니다."
    else:
        msg = f"💎 **{end_date.strftime('%Y-%m-%d')} 돌파 임박 정예 종목** 💎\n"
        msg += "*(흑자+유동성+OBV우상향+정배열+전고점근접)*\n\n"
